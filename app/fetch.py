...
class DomainCatcher:
    ...
    def __parse(self, table):
        ...
        domain_response["domain_pop"] = self.parser_helper(
            table, "field_domainpop")
        domain_response["abirth"] = self.parser_helper(table, "field_abirth")
        domain_response["status_com"] = self.parser_helper(
            table, "field_statuscom")
        domain_response["status_net"] = self.parser_helper(
            table, "field_statusnet")
        domain_response["status_org"] = self.parser_helper(
            table, "field_statusorg")
        domain_response["status_de"] = self.parser_helper(
            table, "field_statusde")
        domain_response["status_ld_registered"] = self.parser_helper(
            table, "field_statustld_registered")
        domain_response["related_cnobi"] = self.parser_helper(
            table, "field_related_cnobi")
    ...
    def thread_func(_func, _list):
        thread_list = [threading.Thread(
            target=_func, args=(x, )) for x in _list]
